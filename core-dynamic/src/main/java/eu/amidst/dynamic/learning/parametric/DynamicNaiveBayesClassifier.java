...
public class DynamicNaiveBayesClassifier {
    ...
    /** Represents the Dynmaic Naive Bayes Classifier model, which is
     considered as a {@link DynamicBayesianNetwork} object. */
    DynamicBayesianNetwork bnModel;

    /** Represents the parallel mode, which is initialized as true. */
    boolean parallelMode = true;

    /** Represents whether the children will be connected temporally
     or not, which is initialized as false. */
     ...
     * @param parallelMode a {@code boolean} that is equal to true if
    the parallel mode is supported, and false otherwise.
     */
     ...
     * @return a {@code boolean} that is equal to true if the children
    are connected temporally.
     */
    ...
    /**
     * Sets the temporal connection between children for this
     DynamicNaiveBayesClassifier.
     * @param connectChildrenTemporally a {@code boolean} that is
    equal to true if the children are connected temporally, and false
    otherwise.
     */
    public void setConnectChildrenTemporally(boolean
    ...
    //TODO: Consider the case where the dynamic data base have TIME_I and SEQ_ID

    /**
     * Sets the ID of the class variable.
     * @param classVarID an {@code int} that represents the ID of the
    class variable.
     */
    ...
    /**
     * Returns this DynamicNaiveBayesClassifier considered as a
    ...
    /**
     * Learns the dynamic graphical structure for this
     DynamicNaiveBayesClassifier given an input data stream.
     * @param dataStream a {@link DataStream} of {@link DynamicDataInstance}s.
     * @return a {@link DynamicDAG} object.
     */
    private DynamicDAG
    dynamicNaiveBayesStructure(DataStream<DynamicDataInstance>
                                       dataStream){

        DynamicVariables dynamicVariables = new
                DynamicVariables(dataStream.getAttributes());
        Variable classVar =
                dynamicVariables.getVariableById(this.getClassVarID());
    ...
    public void learn(DataStream<DynamicDataInstance> dataStream){
        ParallelMaximumLikelihood parallelMaximumLikelihood = new
    ...
    public static void main(String[] args) throws IOException {
        ...
        DataStream<DataInstance> dataStream =
                sampler.sampleToDataStream(sampleSize);
        DataStreamWriter.writeDataToFile(dataStream, file);

        DataStream<DynamicDataInstance> data =
                DynamicDataStreamLoader.loadFromFile(file);

        for (int i = 1; i <= 1; i++) {
            DynamicNaiveBayesClassifier model = new
                    DynamicNaiveBayesClassifier();
            model.setClassVarID(data.getAttributes().getNumberOfAttributes()
                    - 1);
