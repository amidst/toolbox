...
\begin{algorithm}[H]
 \KwData{$sortedList$: sorted list of all scores in a stream, $p$: number of positives in sorted list, $n$: number of negatives in sorted list}
 \KwResult{Prequential $AUC$ at a given time step}
$c \leftarrow 0, AUC \leftarrow 0$;\\
\ForAll{scores $s \in sortedList$}{
\If{isPositive($s$)}{
$c \leftarrow c + 1$
}\Else{
$AUC = AUC + 1$;
...
\todo{Motivate AUC with concept drift}
...
\todo{potentially remove this lag.}

\begin{algorithm}[H]
 \KwData{$sortedList$: sorted list of scores in window, $p$: number of positives in sorted list, $n$: number of negatives in sorted list}
 \KwResult{Prequential $AUC$ at a given time step}
$c \leftarrow 0, AUC \leftarrow 0$;\\
\ForAll{scores $s \in sortedList$}{
\If{isPositive($s$)}{
...
\todo{Sigve says: Maybe discuss stationary explanatory variables with consept drift versus non stationary explanatory variables with concept drift.}
