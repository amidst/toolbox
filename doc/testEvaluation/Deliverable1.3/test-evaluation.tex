...
When the classification is a binary, such as classifying cats and non-cats, the confusion matrix becomes two dimensional as shown in \ref{tab:binary}.  In binary classification, it is common to introduce positives and negatives, instead of the class labels.  A true positive is therefore an actual cat that has been predicted to be a cat by the classifier of interest.  False positives, true negative and false negatives are defined in an equivalent manner.  The results are commonly shown in a confusion table (see table \ref{tab:confusionTable}), which is not the same as a confusion matrix.  
...
\todo{Possibly add a bit more about other measures based on the confusion table.}
