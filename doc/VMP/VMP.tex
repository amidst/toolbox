...
Let $X$ be a multinomial variable with $k$ state spaces, where $k \geq 2$, and let $\mathbf{Y} =\{Y_1,\ldots,Y_n\}$ denote the set of parents of $X$, such that all of them are multinomial. Each parent $Y_i$ has $r_i$ state spaces where  $1 \geq i \geq n$ and $r_i \geq 2$. A parental configuration for $X$ is then a set of $n$ elements $\{Y_1 = {y_1}^{s_1}, \ldots, Y_i = {y_2}^{s_i},\ldots, Y_n = {y_n}^{s_n} \}$ such that $1\leq s_i \leq r_i$. Let $q = r_1 \times \ldots \times r_n$ denote the total number of parental configurations for $X$, and let $\mathbf{y}^l$ denote the $l^{th}$ parental configuration such that $1 \leq l \leq q$.
...
&=&
\begin{pmatrix}
m^{\mathbf{Y}}_1 \cdot \theta_{11}  + m^{\mathbf{Y}}_q \cdot \theta_{1q} \\
\vdots \\
m^{\mathbf{Y}}_1 \cdot \theta_{k1}  + m^{\mathbf{Y}}_q \cdot \theta_{kq}
\end{pmatrix}^T
\begin{pmatrix}
I(X=x^1) \\
\vdots \\
