...
\begin{pmatrix}
\frac{-1}{2\sigma^2} &=& \theta_{\mbox{-}1} \\
\frac{\beta}{\sigma^2} &=& \theta_{\beta} \\
\frac{-\beta^T \beta}{2\sigma^2} &=& \theta_{\beta^T \beta} \\
\end{pmatrix}
\begin{pmatrix}
XX^T   &=& \E(XX^T)\\
Y'X   &=& \E(YX)\\
Y'Y'^T   &=& \E(YY^T)\\
\end{pmatrix}
- (\ln{\sigma}) + \frac{1}{\ln{\sqrt{2\mu_{X|Y}}}}
\end{eqnarray*}
...
\begin{pmatrix}
1 & Y_1   & \cdots & Y_n\\ 
Y_1&Y_1Y_1 & \cdots & Y_1Y_n\\
Y_2&Y_2Y_1 &\cdots & Y_2Y_n\\
\vdots\\
Y_n&Y_nY_1 &\cdots &Y_nY_n\\
\end{pmatrix}
\end{eqnarray*}
...
\item FIRST STEP:
\begin{eqnarray*}
\mu_X &=& E(X) = E(Y'X)_{[1,r]}\\
\mu_\mathbf{Y} &=& \E(Y) = \E(YY^T)_{[1,n]}\\
\Sigma_{XX} &=&  \E(XX^T) - \E(X)^2 = \E(XX^T) - \E(YX)_{[1,r]}^T \E(YX)_{[1,r]} \\
\Sigma_{\mathbf{YY}} &=&  \E(YY^T) - \E(Y)^2 = \E(YY^T) - \E(YY^T)_{[n,1]} \E(YY^T)_{[1,n]} \\
\Sigma_{X\mathbf{Y}} &=&  \E(YX)^T - \E(X)\E(Y) = \E(YX)^T - \E(YX)_{[1,r]} \E(YY^T)_{[n,1]} \\
\Sigma_{\mathbf{Y}X} &=&  \E(YX) - \E(Y)\E(X) = \E(YX) - \E(YY^T)_{[n,1]} \E(YX)_{[1,r]}
\end{eqnarray*}
