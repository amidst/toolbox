...
\caption{\label{Figure:DaimlerSensorModelling} BN fragment for modeling of sensorâ€™s uncertainties with a discrete ``measurement'' variable.}
\end{center}
\end{figure}
...
\caption{\label{Figure:DaimlerLE} BN fragment for modeling of sensorâ€™s uncertainties with a discrete ``measurement'' variable.}
\end{center}
\end{figure}
...
Figure \ref{Figure:daimlerTPlot} shows the evolution on time for velocity and offset in an OBJ\_CutOut and OBJ\_CutIn manoeuvres. The black line corresponds to the values considered from the point of view of the EGO car (left side) and the green line to the measures from the point of view of the right part of the OBJ car. The vertical bar indicates the moment in which the manoeuvre has been recognised by the static OOBN. By taking the temporal properties of the data into account on the model, we should be able to predict the manoeuvre earlier on time. Ideally, the manoeuvre should be detected as soon as the lateral offset and velocity for the OBJ start to increase. 

\begin{figure}[tb]
\begin{center}
\includegraphics[scale=0.65]{./figures/LE_EGO_L_LE_OBJ_L_OBJCut}
...
In order to corroborate the validity of this BN fragment, we have analysed the hypothesis $O(t) - O(t-1) = v(t-1)·dt +N$ on our data. Figure \ref{Figure:daimlerVvsOffs} shows the plot and contour plots for $v(t-1)$ and $O(t) - O(t-1)$, where we observe a linear correlation. 
...
\begin{figure}
  ...
  \renewcommand{\arraystretch}{0.02}
    \begin{tabular}{cc}
    \includegraphics[width=50mm]{figures/OBJplotSerie7.pdf}&
    \includegraphics[width=50mm]{figures/OBJcontourSerie7.pdf}\\
  \end{tabular}
...
\begin{figure}
  \centering
    \begin{tabular}{cc}
    \includegraphics[width=60mm]{figures/CorrOBJ_R50Offs-crop.pdf}&
    \includegraphics[width=60mm]{figures/CorrOBJ_R50Vel-crop.pdf}\\
        \includegraphics[width=60mm]{figures/PcorrOBJ_R5Offs-crop.pdf}&
    \includegraphics[width=60mm]{figures/PcorrOBJ_R5Vel-crop.pdf}\\
  \end{tabular}
