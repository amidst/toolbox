...
Section \label{SubSection:GraphicalNotation} presents the graphical notation. Next, Section \ref{SubSection:HybridBNs}
...
The models presented in this deliverable, unless otherwise stated, will be parameterized according to the conditonal
linear Gaussain model \cite{Lauritzen1992,LauritzenJensen2001}. Some of the advantages of the CLG approach include
simplicity and the support of exact inference. Its main drawbacks lies, however, in the structural limitations it
imposes on the BN and the Gaussian assumption being made. The next section touches upon other alternatives to model
complex empirical distributions that can offer a higher expressive power but, at the same time, contribute to more complicated inference processes. A more detailed analysis covering these topics can be found in the Deliverables 3.1 (more specifically about inference in hybrid and dynamic models) and 4.2 (about learning in the context of the AMIDST framework).
 

\subsection{Modelling complex empirical distributions}


A first alternative to overcome some of the structure limitations of CLGs is to allow more freedom in the choice of
distribution families by resorting to approximate inference algorithms, such as  Gibbs sampling \cite{Geman1984,
  hrycej1990gibbs} and variational inference \cite{Jordan1999}. This could, for example, include the use of logit and probit distributions for parametrizing networks containing discrete children with continuous parents.

A second approach consists in ``transforming'' the original BN model into an approximate one, for which exact inference
algorithms can be applied. This can be achieved, for instance, either by discretizing the continuous variables
\cite{KozlovKollerUAI97} or using transformations with more expressive power such as mixtures of truncated basis
functions (MoTBFs)~\cite{Langseth12}. The MoTBF~\cite{Langseth12} framework was proposed as a general framework for handling hybrid Bayesian
networks with no structural restrictions, unlike the CLG model. MoTBFs are furthermore closed under addition, multiplication, and integration, which facilitates the use of efficient inference methods like the Shenoy-Shafer architecture~\cite{She90} or variable elimination algorithms~\cite{Zha96}. 

An MOTBF can be obtained either by translating a known distribution or learning it directly from data. In the latter
case, several approaches have been investigated utilizing either a proxy representation of the data, e.g., in the form
or a kernel density estimate, or through direct maximum likelihood-based approaches. General MOTBF learning is, however,
more computationally expensive than the models  previously mentioned. 
