...
In the following sub-sections, we present the simplest instantiations of a DBN, namely, hidden Markov models and Kalman filters.

\subsubsection{Hidden Markov models}

A hidden Markov model (HMM) is the simplest DBN with both hidden and observed variables, in which the state of the process is represented by a single discrete variable. This discrete state variable, which can be in reality the combination of several discrete variables, represents the different possible states of the target problem. Figure \ref{Figure:HMM} shows a graphical example of a HMM\footnote{Although the observed variables are represented as discrete, they can be a combination of discrete and continuous variables.}. The advantage of having a single state variable is that all the computations reduce to simple matrix-vector operations.
...
We will represent the sensor (or observation) model by $P(\bm Y_t|\bm X_t)$. Most of our models, although not all of them (Cajamar will be an exception, as shown in Section \ref{Section:CajaMarModels}), will assume that the current state is sufficient to generate the current sensor values. This is referred to as the \textit{sensor Markov assumption}, that is, $P(\bm Y_t \mid \bm X_{0:t}, \bm Y_{0:t-1}) = P(\bm Y_t \mid \bm X_t)$.

By considering the prior probability distribution at time $0$, denoted $P(0)$, we can specify the complete joint distribution over all the variables for any time $t$ as follows:

\begin{equation}
P(\bm X_{0:t},\bm Y_{1:t}) = P(\bm X_0) \prod_{i=1}^t{P(\bm X_i| \bm X_{i-1})P(\bm Y_i|\bm X_i)}
\end{equation}

Although most of our models will fit into this description of observed and hidden (state) variables, there will be cases in which the transition model takes place in observed variables (the case of Cajamar). In general, this simplifies the learning and inference processes of the problem.

\subsubsection{Kalman filters}
Similar to the extension of the static BN model to hybrid domains, DBN models have been likewise extended to continuous and hybrid domains. In purely continuous domains, where the continuous variables follow linear Gaussian distributions, the DBN corresponds to (a factorized version of) a Kalman filter (KF). The structure of a KF is exactly the same as the one displayed in Figure \ref{Figure:HMM} for the HMM, however all the variables should be continuous. In this case, the state variables can be a combination of continuous variables with different (linear) dependences. In KF models, the dynamics of the process are assumed to be linear. When modelling non-linear domains, the dynamics and observational distributions are often approximated through, for e.g. the extended Kalman filter, which models the system as \textit{locally} linear in the mean of the current state distribution. 
...
DBN in general can model arbitrary distributions. An example is presented in Figure \ref{Figure:DBN} where discrete random variables could even have continuous parents. However, in our models, we will try to remain in the CLG family and avoid thereby having discrete children with continuous parents. The probability distributions in the AMIDST models can be parametrized as follows (read notation as parents $\rightarrow$ children):
...
\item Discrete $\rightarrow$ Continuous: distributed as a mixture of Gaussians (the number of components increases exponentially in time if the discrete parent variables are linked over time). 
\end{itemize}

The DBN in Figure \ref{Figure:DBN} satisfies the first-order Markov property, and hence can be sufficiently represented by showing two time slices. These restricted DBN are often refered to as \textit{2-time slice DBN} and usually defined by an initial model representing the initial joint distribution of the process and a transition model (as explained above). 

\begin{figure}[ht!]
\begin{center}
\includegraphics[scale=0.45]{./figures/PreliminariesDBN}
\caption{\label{Figure:DBN}An example of a BN structure corresponding to an unrestricted DBN.}
\end{center}
\end{figure}

%- Mention: finite vs infinite horizone?

Note that HMM, KF or SKF are particular cases of DBN. In fact, every HMM can be represented as a DBN with a single state variable, and every discrete DBN can be represented as a HMM combining all hidden variables into a single state variable. However, it is often preferred to factorise the joint distribution taking the structure of the network into account, in order to take advantage of the sparseness in the model. Likewise, every KF can be represented as a DBN with linear Gaussian conditional distributions, but obviously not every continuous DBN can be represented as a KF, since the latter is limited to single multivariate Gaussian distributions to model the current state distribution. The advantage of using HMM and KF models is that they can benefit from very efficient implementations by using matrix operations.

DBN obviously share the computational difficulties of regular BNs. Nevertheless, in the dynamic case, we are also often faced with the entanglement problem, i.e., after a certain time step, all variables describing the current system state will become dependent, so that the exact probability distribution could not be represented over the current state (the belief state) in a compact factorized form. Well-known algorithms have been defined for learning and inference (including the most likely explanation). Due to the entanglement problem, approximate methods are usually used including approximate factorizations of the joint probability distribution describing the system state \cite{BoyenKoller1998}, as well as sampling based techniques in the form of particle filtering \cite{Doucet2000}.
