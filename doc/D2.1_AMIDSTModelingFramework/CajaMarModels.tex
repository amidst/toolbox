...
In the case of past paying behaviour, the attributes refer to information such as ????. \textcolor{red}{These attributes are updated monthly} and the last 36 months from day $T$ are considered when building the evaluation data set. So, if there are $P$ variables referring to the paying behaviour of a customer, $36\cdot P$ variables are included as information about the past paying behaviour.  
...
\item \textbf{Model Evaluation Data Set:} This data set contains a record for every client to be evaluated at that day $E$. It is built in the same way than the evaluation data set. They contain the same set of predictive variables, with the main difference that they refer to different time points.  As in any standard evaluation data set in supervised classification settings, each record will contain the values of the predictors variables for the different customers. As in the training set, the predictor variables refer to the financial activity and paying behaviour of these clients in the recent past and their socio-demographic data. 
...
The variables within a box can be connected (e.g. according to a tree structure and, globally, conforming a TAN). \textcolor{blue}{Figure X shows how indeed there exist at least pairwise dependences between some of the variables in the data for a particular day.}
...
variable at time $t$, denoted as $X_t$, is linked to the same variable at time $t-1$, $X_{t-1}$,
