...
There are two tasks to be solved for Cajamar's Use Case~\cite{Fer14b}. The main one is the estimation of the \emph{probability of default}, defined as the probability that a customer will end up as defaulter within the next two years. The second task consists in obtaining good customer profiles in terms of risk, so that marketing campaigns can be specifically targeted to these low risk customers. 
...
Additionally, they will select a subset of predictive variables $\Y$ considered relevant from the whole set $\X$ to be part of the final profile for a specific campaign. Mainly  socio-demographic variables will take part in this set, for example, the age of a client is a relevant predictor when designing campaigns to attract customers for a death insurance. 
...
Note that MAP operation will be performed over a model containing the whole set of variables $\X$. There is an open issue when performing MAP inference within continuous domains, which is the definition of the intervals of the continuous variables to conform the profiles. This issue is further considered in the discussion section. 

Let see a running example to clarify the process. Let consider a simple case in which three predictors $\Y$ are considered relevant for a specific campaign: \emph{Age}, \emph{Marital Status} and \emph{Gender}. Let consider also the restriction $\X$ = Age > 50 coming from the marketing department.  After carrying out the computation in Eq.~\ref{eq:MAP}, this may results in the following most probable explanation: clients in the range \emph {[60-70] years}, \emph{single status} and \emph{male}. This procedure will be used to get the $k$ best explanations or in other words the best profiles.   
