...
\ln p(\bm x | \bm y) = \theta_c(\bm y)^Ts(\bm x) - A_c(\theta(\bm x)) + h(\bm x) 
\end{equation}

\item[Posterior form (P-form):]
\begin{equation}
\label{Equation:EqCED}
\ln p(\bm x | \bm y) = \theta_p(\bm x)^Ts(\bm y) - A_p(\theta(\bm y)) + h(\bm x) 
\end{equation}

\item[Full form (F-form):]
\begin{equation}
\label{Equation:EqCED}
\ln p(\bm x | \bm y) = \theta_f^Ts(\bm x,\bm y) - A_f(\theta) + h(\bm x) 
\end{equation}
...
%------------------------------------------------------------------------------------------------------
\section{Representing Bayesian networks as conjugate exponential models}\label{Section:CEFBN}
%------------------------------------------------------------------------------------------------------
...
We start this section defining conjugate exponential Bayesian networks (ce-BN) . 
...
Starting from Equation \ref{Equation:NaturalToMoment} and applying Theorem \ref{Equation:CEFSS}, the vector of moment parameters in a ce-BN decomposes as follows:
...
\noindent where $\mu_i= \int s(X_i,Pa(X_i))~p(X_i,Pa(X_i))d\bm X$. I.e., the global moment parameter of a ce-BN locally decomposes into local moment parameters associated to local marginal probabilities. Note here that in order to compute the local marginal probability $p(X_i,Pa(X_i))$ we need to perform inference over the whole BN.
