 ...
 */
public class MAPFlink_PrecisionAnalysisHUGIN {
...
//         DUMB EXECUTION FOR 'HEATING UP'
                    ...
                    try {
                        ...
                        Domain huginBN = BNConverterToHugin.convertToHugin(bn);
                        huginBN.compile();
                        System.out.println("HUGIN Domain compiled");

                        evidence.getVariables().forEach(variable -> {
                            if (variable.isMultinomial()) {
                                try {
                                    ((DiscreteNode) huginBN.getNodeByName(variable.getName())).selectState((int) evidence.getValue(variable));
                                } catch (ExceptionHugin e) {
                                    System.out.println(e.getMessage());
                                }
                            } else if (variable.isNormal()) {
                                try {
                                    ((ContinuousChanceNode) huginBN.getNodeByName(variable.getName())).enterValue(evidence.getValue(variable));
                                } catch (ExceptionHugin e) {
                                    System.out.println(e.getMessage());
                                }
                            } else {
                        ...
                        NodeList hugin_MAPVariables = new NodeList();

                        //System.out.println(huginBN.getNodes().toString());
                        huginBN.getNodes().stream().filter(node -> {
                            try {
                                return varsInterest.contains(bn.getVariables().getVariableByName(node.getName()));
                            } catch (ExceptionHugin e) {
                                System.out.println(e.getMessage());
                                return false;
                            }
                        }).forEach(hugin_MAPVariables::add);
...
//                System.out.println("HUGIN MAP Variables:" + classVarReplications.toString());
                        ...
                        huginBN.findMAPConfigurations(hugin_MAPVariables, 0.05);
                        ...
                        //                }
                        int[] sequence_Hugin = huginBN.getMAPConfiguration(0);
                    ...
                    Assignment mapEstimate_Hugin = distributedMAPInference.getEstimate();
                    System.out.println("MAP estimate  (HUGIN): " + mapEstimate_Hugin.outputString(varsInterest));
