 ...
 */
public class EF_TruncatedExponentialIntervals extends EF_TruncatedUnivariateDistribution {
     ...
     */
    public EF_TruncatedExponentialIntervals(Variable var1) {
     ...
     */
    public EF_TruncatedExponentialIntervals(Variable var1, double initialDelta) {
    ...
    public EF_UnivariateDistribution deepCopy(Variable variable) {
        EF_TruncatedExponentialIntervals copy = new EF_TruncatedExponentialIntervals(var);
...
//            this.momentParameters.set(0, Math.exp(delta) / (Math.exp(delta) - 1) - 1 / delta);
    ...
    public double computeLogNormalizer() {
        ...
        if (delta>100){
            ...
            if (delta>0)
                return Math.log(Math.exp(delta * this.upperInterval) - Math.exp(delta * this.lowerInterval)) - Math.log(delta);
            else
                return Math.log(-Math.exp(delta * this.upperInterval) + Math.exp(delta * this.lowerInterval)) - Math.log(-delta);
