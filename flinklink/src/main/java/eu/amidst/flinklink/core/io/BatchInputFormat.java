 ...
 * {@link #readRecord(String, byte[], int, int)} method.
 * <p>
 * The default delimiter is the newline character {@code '\n'}.
 */
class BatchInputFormat extends FileInputFormat<String> {
    ...
    public BatchInputFormat() {
        super();
    }

    protected BatchInputFormat(Path filePath, int batchSize) {
        super(filePath);
     ...
     *
     * @param reusable An optionally reusable object.
    ...
    @Override
    public String nextRecord(String record) throws IOException {
        if (readLine()) {
            StringBuffer buffer = new StringBuffer();

            int count = 0;
            do{
                String line = readRecord(record, this.currBuffer, this.currOffset, this.currLen);
                if (!skipHeader(line)){
                    buffer.append(line+"|");
                    count++;
                }
            }while(count<batchSize && readLine());
    ...
    private boolean skipHeader(String s)  throws IOException {
        if (s.isEmpty() ||
                s.startsWith("%") ||
